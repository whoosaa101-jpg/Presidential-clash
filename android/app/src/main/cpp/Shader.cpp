...
Shader *Shader::loadShader(
    ...
    if (program) {
        ...
        if (linkStatus != GL_TRUE) {
            ...
            GLint modelPositionUniform = glGetUniformLocation(
                    program,
                    modelPositionUniformName.c_str());

            // Only create a new shader if all the attributes are found.
            if (positionAttribute != -1
                && uvAttribute != -1
                && projectionMatrixUniform != -1
                && modelPositionUniform != -1) {
...
void Shader::setModelPosition(const Vector3 &modelPosition) const {
    glUniform3fv(modelPosition_, 1, modelPosition.idx);
}
